...
function displayIssues(issues) {
  ...
  issues.forEach((issue) => {
    const card = document.createElement("div");
    card.className = "bg-white shadow-sm hover:shadow-md transition-shadow border border-gray-100 rounded-lg p-6 flex flex-col justify-between cursor-pointer";
    const statusBorder = issue.status === "open" ? "border-t-green-500" : "border-t-purple-500";

    card.className = `bg-white shadow-sm hover:shadow-md transition-shadow border border-gray-100 border-t-4 ${statusBorder} rounded-lg p-6 flex flex-col justify-between cursor-pointer`;
    card.onclick = () => openModal(issue.id);
